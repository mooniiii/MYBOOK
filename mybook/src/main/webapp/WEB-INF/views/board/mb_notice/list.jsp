                ...
                <c:choose>
	                ...
	                <c:otherwise>
						<div class="registerBtn">
                        	<button><a href="/board/mb_notice/register">글쓰기</a></button>
                    	</div>
					</c:otherwise>
				</c:choose>
                        ...
                        <ul>
                        	<c:forEach var="boardlist" items="${list}">
	                            <li>
	                                ...
	                                <p class="title"><a href="/board/mb_notice/read?bno=${boardlist.bno}">${boardlist.title}</a>[${boardlist.replyCnt}]</p>
                    ...
                    <div class="search">
						<div class="searchIn">
							<form id="searchForm" action="/board/mb_notice/list" method="get">
                    ...
                    <div class="pager">
                            <ul>
                            	<c:if test="${pageMaker.prev}">
                            		<li><a href="/board/mb_notice/list?pageNum=${pageMaker.startPage - 1}&amount=${pageMaker.cri.amount}" class="prev"></a></li>
                            	</c:if>
                            	<c:forEach var="num" begin="${pageMaker.startPage}" end="${pageMaker.endPage}">
	                                <li class="${pageMaker.cri.pageNum==num?'active':''}">
	                                	<a href="/board/mb_notice/list?pageNum=${num}&amount=${pageMaker.cri.amount}">${num}</a>
	                                </li>
                                </c:forEach>
                                <c:if test="${pageMaker.next}">
                                	<li><a href="/board/mb_notice/list?pageNum=${pageMaker.endPage + 1}&amount=${pageMaker.cri.amount}" class="next"></a></li>
                                </c:if>
                            </ul>
                    </div> <!--.Pager-->
            </div> <!--.board-->
